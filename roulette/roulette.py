...
class Roulette(BaseCog):
    ...
    async def hitme(self, ctx: commands.Context):
        ...
        channel_list: list[str] = await self._config.guild(
            ctx.guild
        ).roulette_channels()
        ...
        channel_to_use: discord.TextChannel = await ctx.guild.fetch_channel(
            int(channel_to_use_id)
        )
        ...
        random_day = first_day_in_channel + dt.timedelta(
            days=random.randint(0, days_since)
        )
    ...
    async def find_media_from_day(
        self, channel: discord.TextChannel, day: dt.date
    ) -> list[discord.Attachment]:
        media: list[discord.Attachment] = []
        chunk = [
            message
            async for message in channel.history(
                limit=100, around=dt.datetime(day.year, day.month, day.day)
            )
        ]
